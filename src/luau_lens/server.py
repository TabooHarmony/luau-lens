...
def check_code(code: str, filename: str = "snippet.luau") -> dict:
    """Type-check and lint a Luau code string. Always call this after writing or modifying Luau code to catch type errors, unknown properties, deprecated patterns, and unused variables before runtime.

    Works with Roblox API types (Instance, Part, Color3, RemoteEvent, etc.) out of the box. No setup needed beyond installing this MCP server.
...
def check_file(filepath: str) -> dict:
    ...
    Detects and respects existing .luaurc and selene.toml in the file's directory if present.
...
def check_project(directory: str) -> dict:
    ...
    Detects and respects existing .luaurc and selene.toml in the project root if present.
